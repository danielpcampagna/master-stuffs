...
def get_structured_final_text():
    ...
    with open("data/03-final-answer/final-result-v2.full.md.json") as f:
        return json.load(f)
...
Summarize the text in the JSON keeping each study in a separete unique largue paragraph. Each paragraph you produce should contains the dicussion about the insufficiency of this study to address the provided questions propertly, present in the JSON.

Your final text should be a scientific text, in markdown format. This text must seem to be produced by a humam; hence, it follow the rules:
...
4. If you feel a paragraph is too large, break it in to small ones.
    ...
    with open("data/03-final-answer/final-result-v2.summary.md", "w") as f:
        f.write(summary)
